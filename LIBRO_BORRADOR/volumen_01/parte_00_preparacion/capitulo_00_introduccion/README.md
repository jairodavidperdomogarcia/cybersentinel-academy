...
<div class="quiz-block" data-quiz-id="cap00-immersion">
  <p><strong>🛡️ ¿Qué mito de seguridad se acaba de romper?</strong></p>
  <div class="quiz-options">
    ...
    <button class="quiz-option" data-option-id="b" data-correct="true">
      <strong>B. "Seguridad por Oscuridad" (Soy muy pequeño para ser blanco)</strong><br>
      <em>Correcto. Los ataques automatizados barren todo internet; no les importa quién eres, solo si eres vulnerable.</em>
    </button>
    <button class="quiz-option" data-option-id="c">
      <strong>C. "El firewall bloquea todo el tráfico externo"</strong><br>
      <em>Incorrecto. El servidor necesitaba estar expuesto para las citas, el firewall permitió el tráfico legítimo (y el ilegítimo por el mismo puerto).</em>
    </button>
  </div>
...
```mermaid
flowchart LR
    P["🛡️ PREVENIR<br/>(Evitar entrada)"] --> D["👁️ DETECTAR<br/>(Saber si entraron)"]
    D --> R["🚒 RESPONDER<br/>(Minimizar daño)"]
...
graph TD
    subgraph CID [LA TRÍADA DE SEGURIDAD]
        C(🤐 CONFIDENCIALIDAD<br/>Solo autorizados ven)
        I(✍️ INTEGRIDAD<br/>Nadie altera datos)
        D(⏱️ DISPONIBILIDAD<br/>Accesible siempre)
    end
...
```mermaid
flowchart LR
    User(👤 Usuario) -->|Credenciales| Auth{🔐 AUTENTICACIÓN<br/>¿Eres tú?}
    Auth -- NO --> Block1[⛔ Bloqueado]
    Auth -- SÍ --> AuthZ{🎫 AUTORIZACIÓN<br/>¿Puedes pasar?}
...
```mermaid
flowchart TD
    Attacker(👾 Atacante) --> Layer1
    
    subgraph Layer1 [1. PERÍMETRO]
      FW(🧱 Firewall / VPN)
    end
...
flowchart TD
    ...
    subgraph PERIMETER [🌐 PERÍMETRO DE RED]
        FW["🧱 Firewall<br/>(Bloquea tráfico malicioso)"] --> VPN["🔒 VPN<br/>(Conexión remota segura)"]
    end
    
    subgraph ENDPOINT [💻 DISPOSITIVOS]
        VPN --> AV["🦠 Antivirus<br/>(Escanea archivos)"]
    end
    
    subgraph IDENTITY [🆔 IDENTIDAD]
        AV --> MFA["🔑 Autenticación MFA<br/>(Verifica identidad)"]
        MFA --> ACL["📝 Control de Acceso<br/>(Qué puede hacer cada uno)"]
    end
    
    subgraph DATA [💾 DATOS Y RESILIENCIA]
        ACL --> Enc["🔐 Encriptación<br/>(Datos ilegibles)"]
        Enc --> Mon["👁️ Monitoreo<br/>(Detecta sospechosos)"]
        Mon --> Backup["💾 Backups<br/>(Recuperación si todo falla)"]
    end
...
| Concepto | Costo Típico | Impacto |
| :--- | :--- | :--- |
| **Inversión en Seguridad** | **$50-200** /empleado/año | Gasto operativo manejable |
| **Costo de NO tenerlo** | **$4.45 millones** (promedio por breach) | Posible quiebra del negocio |
...
```mermaid
flowchart LR
    Prev(🛡️ PREVENCIÓN<br/>Evitar el ataque) --> Det(👁️ DETECCIÓN<br/>Verlo cuando pasa)
    Det --> Resp(🚒 RESPUESTA<br/>Contener y apagar)
    Resp --> Rec(🩹 RECUPERACIÓN<br/>Volver a operar)
    Rec --> Mej(📈 MEJORA<br/>Aprender lección)
    Mej --> Prev
...
flowchart TD
    subgraph OLD [🏰 MODELO VIEJO: CASTILLO]
      ...
      Muro --> Inside(🔓 RED INTERNA<br/>Confianza Total)
    end
...
<div class="tracker-container" data-chapter-id="cap00">
  <div class="tracker-header">
    <h2>🛡️ CyberSentinel Tracker: Capítulo 00</h2>
    <p>Autoevaluación de Conceptos Clave</p>
  </div>
  ...
  <table class="tracker-table">
    ...
    <tbody>
      <tr>
        <td>Mindset de "Asumir Brecha" (No si pasará, sino cuándo)</td>
      ...
      <tr>
        <td>Comprensión de "Superficie de Ataque"</td>
        <td class="tracker-cell" data-status="empty">
          <span class="status-icon">⚪</span>
          <span class="status-text">Pendiente</span>
        </td>
      ...
      <tr>
        <td>Ética Hacker (White Hat vs Black Hat)</td>
        <td class="tracker-cell" data-status="empty">
          <span class="status-icon">⚪</span>
          <span class="status-text">Pendiente</span>
        </td>
